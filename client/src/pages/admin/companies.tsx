...
import {
  ...
  FileText, Settings, User, Percent, Search, Filter, X, Package, Trash2, Save, MapPin, Loader2, Lock, Star, Home, Navigation, Receipt, Info, Copy, KeyRound
} from "lucide-react";
...
type TabKey = "basico" | "config" | "financeiro" | "contrato" | "entrega" | "enderecos" | "fiscal";
...
const emptyForm = {
  ...
  contractEndDate: "",
  contractVigencia: "",
  // Dados fiscais por empresa (destinatário NF-e)
  stateRegistration: "",
  addressState: "",
  addressIbge: "",
  regimeTributario: "",
...
interface AddressForm {
  label: string; logradouro: string; numero: string; complemento: string;
  bairro: string; cidade: string; estado: string; cep: string; isPrimary: boolean;
}

const emptyAddr: AddressForm = {
  label: 'Sede', logradouro: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '', cep: '', isPrimary: false,
};

function AddressesTab({ company }: { company: Company | null }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (addrId: number) => apiRequest('DELETE', `/api/companies/${companyId}/addresses/${addrId}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/companies', companyId, 'addresses'] });
  ...
  const setPrimaryMut = useMutation({
    mutationFn: (addrId: number) => apiRequest('PATCH', `/api/companies/${companyId}/addresses/${addrId}/set-primary`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/companies', companyId, 'addresses'] });
    ...
    <div className="space-y-4">
        ...
        <div className="bg-muted/40 border-2 border-primary/30 rounded-2xl p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={addrForm.logradouro}
              ...
              data-testid="input-addr-logradouro"
              className="w-full px-3 py-2 rounded-xl border-2 border-border focus:border-primary outline-none text-sm" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={addrForm.complemento}
                ...
                data-testid="input-addr-complemento"
                className="w-full px-3 py-2 rounded-xl border-2 border-border focus:border-primary outline-none text-sm" />
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => saveMut.mutate(addrForm)} disabled={saveMut.isPending || !addrForm.logradouro || !addrForm.cidade}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10 border-2 border-dashed border-border rounded-2xl">
          ...
          <p className="text-xs text-muted-foreground mt-1">Clique em "Adicionar CEP" para incluir filiais e pontos de entrega</p>
        </div>
      ) : (
        <div className="space-y-3">
          {addresses.map((addr: any) => (
            <div key={addr.id} className={`rounded-xl border-2 p-4 transition-all ${addr.isPrimary ? 'border-yellow-300 bg-yellow-50/50' : 'border-border bg-card'}`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button type="button" onClick={() => { if (confirm('Remover este endereço?')) deleteMut.mutate(addr.id); }}
...
export default function CompaniesPage() {
  ...
  const [tempPasswordModal, setTempPasswordModal] = useState<{ companyName: string; email: string; password: string } | null>(null);
    ...
    <Layout>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {activeTab === "fiscal" && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    data-testid="input-company-state-reg"
              ...
              {/* Configuração fiscal padrão por empresa */}
              <div>
                <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="5102 (global)"
                      maxLength={4}
                      data-testid="input-company-cfop"
                      className="w-full px-4 py-2.5 rounded-xl border-2 border-border focus:border-primary outline-none text-sm"
                    />
                ...
                <div className="pt-4 border-t border-border/50">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {[
                      ...
                      <div key={label} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${ok ? 'bg-emerald-50 text-emerald-700' : 'bg-orange-50 text-orange-600'}`}>
      ...
      {tempPasswordModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setTempPasswordModal(null)} />
          <div className="relative bg-card rounded-3xl shadow-2xl border border-border max-w-md w-full p-8 z-10">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{tempPasswordModal.companyName}</p>
              </div>
            </div>
            ...
            <div className="mb-6 p-4 rounded-2xl bg-amber-50 border border-amber-200">
              ...
              <p className="text-sm text-amber-800">
                Esta senha temporária será exibida <strong>apenas uma vez</strong>. Copie e compartilhe com o cliente. Ele será obrigado a trocá-la no primeiro login.
              </p>
            </div>

            <div className="mb-2">
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Conta de acesso</label>
              <p className="text-sm font-mono font-semibold text-foreground">{tempPasswordModal.email}</p>
            </div>

            <div className="mb-6">
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Senha temporária</label>
              <div className="flex items-center gap-3">
                <code
                ...
                <button
                  ...
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(tempPasswordModal.password).then(() => {
