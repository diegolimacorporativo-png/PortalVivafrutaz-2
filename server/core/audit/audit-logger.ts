...
/**
 * audit-logger — structured security audit records persisted to the database.
 *
 * PURPOSE: Write structured SEC:<action> entries to the systemLogs table via
 * storage.createLog(). Includes anomaly detection (sliding-window counters),
 * role-risk classification, and cross-tenant boundary detection.
 *
 * DISTINCT FROM: server/core/security/securityLogger.ts
 *   That file is the IN-MEMORY security event store (circular buffer, no DB).
 *   It handles real-time observability for the admin security dashboard.
 *   Use THIS file (audit-logger) for compliance/audit trail writes to the DB.
 *   Use securityLogger for operational runtime event buffering.
 *
 * Exports:
 *   logSecurityEvent(payload)  — fire-and-forget DB audit write
 *   classifyRoleRisk(role)     — returns HIGH | MEDIUM | LOW
 *
 * Type exports:
 *   TenantScope, AccessIntent, RoleRiskLevel, SecurityEventPayload
 */
...
export function logSecurityEvent(payload: SecurityEventPayload): void {
  ...
  storage
    ...
    .catch((err: Error) => {
      console.error("[AUDIT_LOGGER] Failed to persist event:", err?.message);
    });
