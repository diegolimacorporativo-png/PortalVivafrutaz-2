...
/**
 * api-rate-limit — simple, generic in-memory IP rate limiter.
 *
 * PURPOSE: Protects unauthenticated API entrypoints from basic flood attacks.
 * Applies only to /api routes; authenticated sessions (userId / companyId) are
 * always bypassed.
 *
 * DISTINCT FROM: server/core/security/rateLimit.ts
 *   That file is the full security rate-limiting suite with multiple
 *   pre-configured limiters (login, NF-e, admin, sensitive actions, etc.),
 *   strategic account bypass, and audit event emission.
 *   Use those limiters for route-level protection.
 *   Use this one only as a coarse entry-point guard.
 *
 * Export: simpleRateLimit — Express middleware
 */

const memory = new Map<string, { count: number; time: number }>();

const WINDOW = 60_000; // 1 min
const LIMIT = 60; // 60 req/min per IP
