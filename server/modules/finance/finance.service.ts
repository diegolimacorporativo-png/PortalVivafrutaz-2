...
// FASE FISCAL 8.1 — a regra cStat → sugestão foi movida para
// `server/services/nfe/nfeErrorHandler.ts` e é injetada diretamente pelo
// repository (`finance.repository.getNfeMotivosRejeicao`). Mantemos o
// service como pass-through para preservar a fronteira HTTP/contrato sem
// duplicar a tabela de tradução em dois lugares.
...
export class FinanceService {
  ...
  // FASE FISCAL 7.9 / 8.1 — pass-through. A sugestão estruturada
  // ({tipo, mensagem}) já vem injetada pelo repository via `nfeErrorHandler`.
  getNfeMotivosRejeicao() {
