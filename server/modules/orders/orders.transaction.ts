...
export interface CriticalTransitionResult {
  ...
  /** Pre-nota number generated by this transition, when applicable. */
  preNotaNumber: string | null;
  /** How many inventory product lines were deducted (APPROVED only). */
  inventoryLinesDeducted: number;
  /** Whether an accounts-receivable record was created (APPROVED only). */
  arCreated: boolean;
...
export async function executeWorkflowTransaction(
  ...
  return db.transaction(async (tx) => {
    ...
    // ── Step 4a: APPROVED — strict stock deduction ──────────────────────────
    ...
    // ── Step 4b: APPROVED — accounts receivable seeding ─────────────────────
    // Financial setup is tied to approval, not to the legacy INVOICED state
    // (which now means "Liberado para entrega"). The existence check keeps
    // approval retries idempotent and prevents duplicate receivables.
    if (to === OrderStatus.APPROVED) {
      ...
      if (existingAR.length === 0) {
        ...
        if (total > 0) {
          ...
          // FASE 1 — Log de aprovação financeira.
          // Marca explicitamente que este AR foi gerado pela automação ligada
          // à transição → APPROVED do pedido (via executeWorkflowTransaction).
    ...
    if (to === OrderStatus.SHIPPED) {
      ...
      // which indicates the normal approval financial setup was bypassed. This guard never
      ...
      // logic is inline in the APPROVED block above and must not be duplicated.
      try {
        const existingAR = await tx
          .select({ id: accountsReceivable.id })
          .from(accountsReceivable)
