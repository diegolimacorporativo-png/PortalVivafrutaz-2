  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        (req as any).user = currentUser ? { id: currentUser.id, role: currentUser.role, tenantId: currentUser.empresaId != null ? String(currentUser.empresaId) : null } : undefined;
        ...
        const recentAlerts = effectiveTenantId
          ? await db.select().from(systemAlerts).where(or(eq(systemAlerts.tenantId, effectiveTenantId), isNull(systemAlerts.tenantId))).orderBy(desc(systemAlerts.createdAt)).limit(20)
          : await db.select().from(systemAlerts).where(isNull(systemAlerts.tenantId)).orderBy(desc(systemAlerts.createdAt)).limit(20);
        const dbPolicies = effectiveTenantId
          ? await db.select().from(systemPolicies).where(or(eq(systemPolicies.tenantId, effectiveTenantId), isNull(systemPolicies.tenantId))).orderBy(desc(systemPolicies.priority))
          : await db.select().from(systemPolicies).where(isNull(systemPolicies.tenantId)).orderBy(desc(systemPolicies.priority));
