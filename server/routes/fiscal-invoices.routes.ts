...
export function register(app: Express) {
  // GET /api/fiscal-invoices — list invoices scoped to the current tenant.
  // currentTenantId() null-safe: MASTER without pinned tenant receives [].
  app.get('/api/fiscal-invoices', requireAuth, requireRole(READ_ROLES), tenantContext, async (req, res) => {
    res.json(await storage.getFiscalInvoices());
  });

  // GET /api/fiscal-invoices/check-duplicate — scoped to current tenant
  app.get('/api/fiscal-invoices/check-duplicate', requireAuth, requireRole(READ_ROLES), tenantContext, async (req, res) => {
  ...
  // GET /api/fiscal-invoices/:id — ownership enforced by tenantId in query
  app.get('/api/fiscal-invoices/:id', requireAuth, requireRole(READ_ROLES), tenantContext, async (req, res) => {
  ...
  // POST /api/fiscal-invoices — requireTenant ensures empresa_id is injected by withTenant()
  app.post('/api/fiscal-invoices', requireAuth, requireRole(WRITE_ROLES), tenantContext, requireTenant, async (req: any, res) => {
  ...
  // DELETE /api/fiscal-invoices/:id — tenantAnd in storage enforces ownership
  app.delete('/api/fiscal-invoices/:id', requireAuth, requireRole(WRITE_ROLES), tenantContext, requireTenant, async (req, res) => {
