 ...
 *   ✅ Antes das 13:00   ✅ Depois das 13:00
...
//
// Regra: prazo = 2 dias úteis antes da entrega, às 16:00 UTC (13:00 BRT)
// Referência: semana de 03–07/08/2026
//
//   Entrega Seg 03/08 → prazo Qui 30/07 às 16:00 UTC
//   Entrega Ter 04/08 → prazo Sex 01/08 às 16:00 UTC
//   Entrega Qua 05/08 → prazo Seg 03/08 às 16:00 UTC
//   Entrega Qui 06/08 → prazo Ter 04/08 às 16:00 UTC
//   Entrega Sex 07/08 → prazo Qua 05/08 às 16:00 UTC
...
// ─── 2. canModify: antes e depois das 13:00 ───────────────────────────────────
//
// Entrega quinta 06/08/2026 → prazo terça 04/08/2026 às 16:00 UTC

describe("canModify — antes e depois do prazo", () => {
  const DELIVERY = "2026-08-06T12:00:00Z"; // quinta
  const DEADLINE  = "2026-08-04T16:00:00Z"; // terça às 16:00 UTC

  test("antes das 13:00 BRT do dia limite → canModify = true", () => {
    const now = utc("2026-08-04T15:59:59Z"); // 12:59 BRT
    const { canModify, reason } = calculateOrderModificationDeadline(DELIVERY, { now });
    assert.equal(canModify, true);
    assert.equal(reason, "");
  });

  test("exatamente às 13:00 BRT (16:00 UTC) → canModify = true (limite inclusivo)", () => {
  ...
  test("1 segundo depois das 13:00 BRT → canModify = false", () => {
    const now = utc("2026-08-04T16:00:01Z"); // 13:00:01 BRT
    const { canModify, reason } = calculateOrderModificationDeadline(DELIVERY, { now });
...
describe("Fim de semana — ignorados no cálculo", () => {
  test("entrega segunda: prazo pula sábado e domingo corretamente", () => {
    ...
    // Portanto prazo = qui 30/07/2026 às 16:00 UTC  ← conforme spec
    const { deadline } = calculateOrderModificationDeadline("2026-08-03T12:00:00Z");
    assert.equal(deadline.toISOString(), "2026-07-30T16:00:00.000Z");
  });

  test("na sexta após o prazo (que era quinta) → canModify = false", () => {
    // Entrega seg 03/08 → prazo qui 30/07/2026 às 16:00 UTC
  ...
  test("na quinta antes das 13:00 BRT → canModify = true", () => {
    // Prazo é quinta 30/07/2026 às 16:00 UTC (13:00 BRT)
    // quinta 30/07 às 16:00 UTC (13:00 BRT) → dentro do prazo
    const now = utc("2026-07-30T15:00:00Z");
    const { canModify } = calculateOrderModificationDeadline("2026-08-03T12:00:00Z", { now });
    assert.equal(canModify, true);
  });
});
...
describe("Mudança de mês", () => {
  test("prazo cai no mês anterior à entrega", () => {
    ...
    assert.equal(deadline.toISOString(), "2026-08-31T16:00:00.000Z");
  });
...
describe("Mudança de ano", () => {
  test("prazo cai no ano anterior à entrega", () => {
    ...
    assert.equal(deadline.toISOString(), "2026-12-31T16:00:00.000Z");
  });
...
describe("Estrutura do retorno", () => {
  ...
  test("deadline é sempre às 16:00 UTC (13:00 BRT)", () => {
    const { deadline } = calculateOrderModificationDeadline("2026-08-06T12:00:00Z");
    assert.equal(deadline.getUTCHours(), 16);
