...
### Auth Response Hardening + IStorage Reconciliation (FASE 7.1)
- **Hotfix de segurança:** o hash da senha (`users.password` / `companies.password`) deixou de ser devolvido em `POST /api/auth/login` e `GET /api/auth/me`. Aplicado tanto no controller modular (`server/modules/auth/auth.controller.ts`) quanto no handler inline em `server/routes/routes.ts` (`/api/auth/me`). Estratégia: destruct `const { password: _pw, ...safe } = obj` antes do `res.json(...)`. Nenhum outro campo, status code, header ou shape de envelope foi alterado.
- **Limpeza de imports em `server/routes/routes.ts`:** removidos `path` (0 usos) e os helpers Drizzle não referenciados (`lte`, `and`, `eq`, `isNull`). Mantidos `gte`, `desc`, `sql` (12 usos), `bcrypt`, `fs`, `db`. Sem mudança de comportamento.
- **Reconciliação `IStorage` ↔ `DatabaseStorage`** em `server/services/storage.ts`: 32 assinaturas que existiam apenas na implementação (Tasks, ClientIncidents, IncidentMessages, InternalIncidents, CompanySettings, AboutUs, SmtpConfig, PushSubscriptions, NotificationSettings, e o módulo Sanitário) foram declaradas no contrato. Apenas tipagem — zero alteração em runtime, zero novos métodos.
- **Validação:** `.local/audit/run-baseline.mjs` re-executado contra os 16 endpoints / 53 cenários. Comparado com `baseline.before.json`: única diferença legítima é a remoção da chave `password` em 4 respostas (`auth.login.success.user`, `auth.login.noAuth.user`, `auth.me.success.user`, `auth.me.crossTenant.company`). Status codes, headers (content-type, cache-control, set-cookie), idempotência, isolamento multi-tenant e shape de erro permanecem idênticos. `noSensitiveLeak` passou de `false` para `true`. TypeScript: 0 erros novos em `storage.ts` e `auth.controller.ts`.
